...
for (bufferNo in 1:length(restorePerBuffer))
{
  ...
  if (nrow(buffer) > 0) #Buffer has restoration projects
  {
    ...
    {
      buffersWRestore = c(buffersWRestore, bufferNo)
...
buffersWRestore = unique(buffersWRestore)
plotRegions(states=T, clipRegions=c(9,10))
plot(restore,add=T,border=NA,col="#00FF0033")
plot(thk99buff,add=T,border=NA,col="black")
plot(thk99buff[buffersWRestore,], add=T,border=NA,col="red")
...
# Reduce Data -------------------------------------------------------------
# Any restore, within 10 years# 
# thk99buff_n = thk99buff_n[((thk99buff_n$YEAR <= 10) | thk99buff_n$RESTORE == 0),]
# HA only (removes buffers w restore that isn't HA), within 10 years
# thk99buff_n = thk99buff_n[((thk99buff_n$YEAR <= 10 & thk99buff_n$HA == 1 ) | thk99buff_n$RESTORE == 0),]
# HA only (removes buffers w restore that isn't HA), within 5 years
# thk99buff_n = thk99buff_n[((thk99buff_n$YEAR <= 5 & thk99buff_n$HA == 1 ) | thk99buff_n$RESTORE == 0),]
...
output = coda.samples.dic(model = model,
                          variable.names=c("b0", paste0("b", params),
                                           paste0("b", restoreParams),
                                           "bHA","bYEAR"),
                          n.iter=20000,
                          thin=1)


MCMCplot(output$samples, ref_ovl = T)
